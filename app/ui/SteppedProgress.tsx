...
export const NewSteps = ({ width, labels, numSteps, stepsComplete }: NewStepsProps) => {
    const stepArray = Array.from(Array(numSteps).keys());

    return (
        <div className={`${width} flex items-center justify-between gap-3`}>
            {stepArray.map((num) => {
                const stepNum = num + 1;
                const isActive = stepNum <= stepsComplete;
                return (
                    <React.Fragment key={stepNum}>
                        <NewStep label={labels[num]} num={stepNum} isActive={isActive} />
                        {stepNum !== stepArray.length && (
                            <div className="w-full h-1 rounded-full bg-gray-200 relative">
                                <motion.div
                                    className="absolute top-0 bottom-0 left-0 bg-sky-300 rounded-full"
                                    animate={{ width: isActive ? "100%" : 0 }}
                                    transition={{ ease: "easeIn", duration: 0.3 }}
                                />
                            </div>
...
const NewStep = ({ label, num, isActive }: NewStepProps) => {
    return (
        <div className="relative flex flex-wrap justify-center items-center gap-2">
            ...
            <div
                className="text-xs w-full whitespace-nowrap">{label}</div>
            {/* {isActive && (
                <div className="absolute z-0 -inset-1.5 bg-indigo-100 rounded-full animate-pulse" />
            )} */}
        </div>
